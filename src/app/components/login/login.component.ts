...
export class LoginComponent {
    ...
    ngOnInit(): void {
        if (!this.userService.isLogged()) {
            const paramCode = this.activatedRoute.snapshot.queryParamMap.get('code')
            const paramState = this.activatedRoute.snapshot.queryParamMap.get('state')
            ...
            if (paramCode && paramState) {
                this.userService
                    ...
                    .subscribe(() => {
                        ...
                        const requestedRoute = localStorage.getItem('requested_uri') ?? ''
